...
class ProformInvoice(SellingController):

	
	def __init__(self, *args, **kwargs):
		super(ProformInvoice, self).__init__(*args, **kwargs)

	def validate(self):
		super(ProformInvoice, self).validate()
	...
	def validate_po(self):
		...
		if self.po_no and self.customer and not self.skip_delivery_note:
			so = frappe.db.sql(
				"select name from `tabProform Invoice` \
			...
			if (
				...
				frappe.msgprint(
					_("Warning: Proform Invoice {0} already exists against Customer's Purchase Order {1}").format(
						so[0][0], self.po_no
					)
				)
			...
			if self.delivery_date:
				for d in self.get("items"):
					...
					if getdate(self.transaction_date) > getdate(d.delivery_date):
						frappe.msgprint(
							_("Expected Delivery Date should be after Proform Invoice Date"),
							indicator="orange",
							title=_("Warning"),
						)
			else:
	...
	def validate_warehouse(self):
		super(ProformInvoice, self).validate_warehouse()
	...
	def validate_with_previous_doc(self):
		super(ProformInvoice, self).validate_with_previous_doc(
	...
	def on_cancel(self):
		...
		super(ProformInvoice, self).on_cancel()
	...
	def check_credit_limit(self):
		# if bypass credit limit check is set to true (1) at Proform Invoice level,
	...
	def check_nextdoc_docstatus(self):
		...
		if linked_invoices:
			...
			frappe.throw(
				_("Sales Invoice {0} must be deleted before cancelling this Proform Invoice").format(
					", ".join(linked_invoices)
				)
			)

	def check_modified_date(self):
		mod_db = frappe.db.get_value("Proform Invoice", self.name, "modified")
		...
		for item in self.items:
			if item.supplier:
				supplier = frappe.db.get_value(
					"Proform Invoice Item", {"parent": self.name, "item_code": item.item_code}, "supplier"
				)
...
def close_or_unclose_sales_orders(names, status):
	if not frappe.has_permission("Proform Invoice", "write"):
		frappe.throw(_("Not permitted"), frappe.PermissionError)

	names = json.loads(names)
	for name in names:
		so = frappe.get_doc("Proform Invoice", name)
		if so.docstatus == 1:
			if status == "Closed":
...
def make_material_request(source_name, target_doc=None):
	...
	doc = get_mapped_doc(
		...
		source_name,
		{
			"Proform Invoice": {"doctype": "Material Request", "validation": {"docstatus": ["=", 1]}},
			"Packed Item": {
...
def make_delivery_note(source_name, target_doc=None, skip_item_mapping=False):
	...
	mapper = {
		"Proform Invoice": {"doctype": "Delivery Note", "validation": {"docstatus": ["=", 1]}},
	...
	target_doc = get_mapped_doc("Proform Invoice", source_name, mapper, target_doc, set_missing_values)
...
def make_maintenance_schedule(source_name, target_doc=None):
	...
	if not maint_schedule:
		doclist = get_mapped_doc(
			...
			source_name,
			{
				"Proform Invoice": {"doctype": "Maintenance Schedule", "validation": {"docstatus": ["=", 1]}},
				"Proform Invoice Item": {
...
def make_maintenance_visit(source_name, target_doc=None):
	...
	if not visit:
		doclist = get_mapped_doc(
			...
			source_name,
			{
				"Proform Invoice": {"doctype": "Maintenance Visit", "validation": {"docstatus": ["=", 1]}},
				"Proform Invoice Item": {
...
def get_events(start, end, filters=None):
	...
	conditions = get_event_conditions("Proform Invoice", filters)

	data = frappe.db.sql(
		"""
		select
			distinct `tabProform Invoice`.name, `tabProform Invoice`.customer_name, `tabProform Invoice`.status,
			`tabProform Invoice`.delivery_status, `tabProform Invoice`.billing_status,
			`tabProform Invoice Item`.delivery_date
		from
			`tabProform Invoice`, `tabProform Invoice Item`
		where `tabProform Invoice`.name = `tabProform Invoice Item`.parent
			and `tabProform Invoice`.skip_delivery_note = 0
			and (ifnull(`tabProform Invoice Item`.delivery_date, '0000-00-00')!= '0000-00-00') \
			and (`tabProform Invoice Item`.delivery_date between %(start)s and %(end)s)
			and `tabProform Invoice`.docstatus < 2
			{conditions}
		""".format(
...
def set_delivery_date(items, sales_order):
	delivery_dates = frappe.get_all(
		"Proform Invoice Item", filters={"parent": sales_order}, fields=["delivery_date", "item_code"]
	)
...
def make_work_orders(items, sales_order, company, project=None):
	"""Make Work Orders against the given Proform Invoice for the given `items`"""
...
def make_inter_company_purchase_order(source_name, target_doc=None):
	...
	return make_inter_company_transaction("Proform Invoice", source_name, target_doc)
...
def create_pick_list(source_name, target_doc=None):
	...
	doc = get_mapped_doc(
		...
		source_name,
		{
			"Proform Invoice": {"doctype": "Pick List", "validation": {"docstatus": ["=", 1]}},
			"Proform Invoice Item": {
...
def update_produced_qty_in_so_item(sales_order, sales_order_item):
	# for multiple work orders against same Proform Invoice item
	...
	frappe.db.set_value("Proform Invoice Item", sales_order_item, "produced_qty", total_produced_qty)
...
def get_work_order_items(sales_order, for_raw_material_request=0):
	...
	if sales_order:
		so = frappe.get_doc("Proform Invoice", sales_order)
...
class ProformInvoice(SellingController):
	@frappe.whitelist()
	def getitems(self):
		doc = frappe.db.get_list("Sales Order", fields=["name","payment_terms_template"])
		for so in doc:
			if(self.sales_order==so.name):
				self.payment_t = so.payment_terms_template
				...
				for mg in doc2.get("payment_schedule"):
